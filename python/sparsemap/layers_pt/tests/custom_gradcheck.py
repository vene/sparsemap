...
def get_analytical_jacobian(input, output, reentrance_tol=1e-4):
    ...
    for jacobian_x, jacobian_reentrant_x in zip(jacobian, jacobian_reentrant):
        if (jacobian_x.numel() != 0 and
                (jacobian_x - jacobian_reentrant_x).abs().max() > reentrance_tol):
            reentrant = False
...
def gradcheck(func, inputs, eps=1e-6, atol=1e-5, rtol=1e-3, raise_exception=True):
    ...
    for i, o in enumerate(output):
        ...
        analytical, reentrant, correct_grad_sizes = get_analytical_jacobian(tupled_inputs, o, reentrance_tol=atol)
